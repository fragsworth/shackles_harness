...
Agent definitions register only at session start; one written mid-session is "not found", so run `agents --write` and commit before the session that drives the round.
...
A crash mid-command is resumed by rerunning `next`; unrecorded work of a pending attempt is refused until you `record` it or run `next --discard`.
