...
`next` does the mechanical steps on the way (CHAT-TO-PLAN-GATE, disabled gates, overridden steps) and LANDING itself, after the `spec-edit` checkpoint when the round edited a spec file: it merges the target, runs verify and the suite (an `L1` or `L2` finding sends the round back to SPEC-TO-IMPLEMENTATION instead), pushes `origin/<mainBranch>` in a worktree round, books the living charge and tags, then prints the POSTMORTEM action (or `done` when POSTMORTEM is overridden), so a plain `next` can land the round; the landing shows as `living_usd` in `spend`, the `LANDED` line in HISTORY and, when `<mainBranch>` is checked out elsewhere, a stderr `note:` to pull it.
