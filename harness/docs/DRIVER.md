...
`next` does the mechanical steps on the way (CHAT-TO-PLAN-GATE, disabled gates, overridden steps) and LANDING itself: it merges the target, runs verify and the suite, pushes `origin/<mainBranch>`, books the living charge and tags, then prints the POSTMORTEM action, so a plain `next` can land the round; the landing shows as `living_usd` in `spend`, the `LANDED` line in HISTORY and, when `<mainBranch>` is checked out elsewhere, a stderr `note:` to pull it.
