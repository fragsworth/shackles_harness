...
A `question` or `blocked` checkpoint leaves the attempt recorded but not accepted: `answer --text` or `approve` (which answers "proceed on your stated assumption") re-runs the producer at the next attempt with the answer as finding O1; at `failure-limit`, `approve` resets the count and the producer re-runs with its open findings, or `override` skips it; at `infra`, `approve` resets the count and `next` reprints the same attempt; at `upstream-plan`, edit PLAN.json in the worktree, then `approve`.
