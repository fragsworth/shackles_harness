...
Interpreting the owner's words is your judgment call: `approve`/`approved` means approved with checkpoints, `delegate`/`delegated` means no review checkpoints, `delegate through STEP` skips the reviews up to and including STEP, `override` plus names skips those steps.
