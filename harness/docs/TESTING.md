...
Level 2, tens of dollars per pass: `sandbox --dir D` builds a temp clone with a local bare origin and the toy project; run a real round there with `run --until done` (gates off, `--delegate`), then with gates on, then answering checkpoints.
