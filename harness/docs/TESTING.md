...
Level 1, cents to dollars per run: `probe --step S [--seed clean|defect] [--agent RUNG] [--budget USD] [--manual] --dir DIR` builds a temp repo in DIR (a new folder: one already holding `repo/` is refused) with the real prose played to just before S, prints the action, runs one real agent (or you spawn it with `--manual`), and `probe --check RESULT_FILE [--dir DIR]` (DIR is found above the result file when omitted) scores contract validity, schema validity, path confinement, the expected verdict and whether one of the planted anchors was quoted; `--changed` probes the steps whose prose drifted; `--seed defect` is refused for a producer step, which has no plant.
Planted defects live in `tests/fixtures/defects/<GATE>/` with `expect.json`, whose `quote_contains` is one anchor or a list of them (any listed anchor counts, since a right FAIL may quote either half of a plant); a clean artifact must PASS and a planted defect must FAIL with the planted text quoted.
